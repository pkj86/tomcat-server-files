...
<c:choose>
	...
	<form id="replyRegForm" action="writeReply.json" method="post">
	<!-- <tr> -->
		<!-- <td align="center"> -->
...
<script type="text/javascript">
	...
	{
		$.ajax({
			url: "${pageContext.request.contextPath}/jspboard/replyList.json",
	...
	function makeReplyList(result)
	{
		...
		for(var i = 0 ; i < result.length ; i++)
		{
			...
			if('${user.name}' == reply.rWriter || '${user.name}' == '관리자')
			{
	...
	$("#replyRegForm").submit(function(){
		...
		$.ajax({
			url: "${pageContext.request.contextPath}/jspboard/writeReply.json",
			type: "post",
	...
	{
		$.ajax({
			url: "${pageContext.request.contextPath}/jspboard/deleteReply.json",
	...
	$("#replyUpdateDiv > button").click(function(){
		...
		$.ajax({
			url: "${pageContext.request.contextPath}/jspboard/updateReply.json",
